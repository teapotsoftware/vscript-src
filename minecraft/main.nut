...
::CollapseDigsite <- function()
{
	...
	local count = EuthanasiaQueue.len()
	foreach (block in EuthanasiaQueue)
	{
		block.Destroy()
	}
	DebugPrint("Brutally euthanized " + count + " edicts")
...
::RenderDungeon <- function(n = 0)
{
	...
	// doing bulk edict allocation here, which can fuck shit up REALLY badly
	// we gotta have plenty of room before we do this
...
OnPostSpawn <- function()
{
	...
	printl("testing")
	local start = Time()
	for (local x = 0; x < 9999; x++)
	{
		for (local y = 0; y < 9999; y++)
		{
			for (local z = 0; z < 9999; z++)
			{
				//print("--> x:" + x + " y:" + y + " z:" + z + " total:" + ((x + 1) * (y + 1) * (z + 1)) + " time: " + (Time() - start))
				print("-" + ((x + 1) * (y + 1) * (z + 1)))
			}
		}
	}
...
Think <- function()
{
	...
	while ((ply = Entities.FindByClassname(ply, "player")) != null)
	{
		...
		local forward = ply.GetForwardVector()
		local mag = 300
		local endpos = eyepos + (forward * mag)
		DebugDrawLine(eyepos, endpos, 255, 0, 0, true, 0.5)
		local frac = TraceLine(eyepos, endpos, ply)
		DebugDrawLine(eyepos, endpos, 255, 0, 0, true, 0.5)
		local tracehit = eyepos + (forward * mag * frac)
		DrawAxes(tracehit)
	}
}
*/
