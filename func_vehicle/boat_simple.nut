...
Think <- function(slopes = false)
{
	...
	if (inputs)
	{
		...
		if (HeldInputs[VEHICLE_LEFT]) TurnAngle -= 1.0
		if (HeldInputs[VEHICLE_RIGHT]) TurnAngle += 1.0

		Speed = Clamp(Speed, -0.5, 1)
		TurnAngle = Clamp(TurnAngle, -5, 5)
	}
	else
	{
	...
	if ("SteeringWheel" in this)
		SteeringWheel.SetAngles(0, 0, TurnAngle * 15)

	OldSpeed = Speed
	TurnAngle = ApproachZero(TurnAngle, 0.5)
